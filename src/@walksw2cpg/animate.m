...
if treadmill % make room for treadmill or overground steps
  xlimit = [-(sl+R)*2 sl*2];    % horizontal
else
  xlimit = [-(sl/2+R) distance];% make room for the whole walk
end    
ylimit = [-floorheight-1.02*L 1.02*L];        % vertical limit
...
for j = 1:numsteps % Loop through each step
  ...
  if(~isempty(falls)&&j>1&&falls(j-1))
     ...
     set(htext, 'String', ['num Falls: ', num2str(sum(falls(1:j-1)))]);
  end
  ...
  for i = 1:framesthisstep % each step has this many frames
    ...
    if(mod(j,2)==0) % 15:com
        set(h(12), 'Color', 'b'); set(h(11), 'Color', 'b');
        set(h(13), 'Color', 'r'); set(h(14), 'Color', 'r');
    else
        set(h(12), 'Color', 'r'); set(h(11), 'Color', 'r');
        set(h(13), 'Color', 'b'); set(h(14), 'Color', 'b');
    end
  ...
  Walk_distance = Walk_distance + (L-R)*sin(xs(end,2))+R*xs(end,2) + ...
