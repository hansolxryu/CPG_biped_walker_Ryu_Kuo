...
floorheight = 0.04; % default, but can be changed as parameter
modellinewidth = 2.5; groundlinewidth = 4; jointsize = 10; footsize = 6;
...
elseif nargin > 1 % optional arguments
  ...
  while length(property_argin) >= 2,
    ...
    switch prop
      ...
      case 'drawFromData'
        drawFromData = size(val,1); % either 0 or num of provided data
        drawData = val;
      case 'treadmillVel'
        treadmillVel = val;    
      case 'falls'
...
if treadmill % make room for treadmill or overground steps
  xlimit = [-(sl+R)*2 sl*2 + 3];    % horizontal
else
  xlimit = [-(sl/2+R) distance+sl+R + 3];% make room for the whole walk
end    
ylimit = [-floorheight-1.02*L 1.6];        % vertical limit
...
ygndoffset = 0; % for point feet
ytileoffset = ygndoffset - 0.5*groundlinewidth*ptstodata;
hgnd = line(xlimit,[1 1]*ygndoffset,'color',[.75 .7 .65],'linewidth',groundlinewidth);

text(-0.5, 1.2, modelLabel, 'fontsize', 22)

text(0+(-0.5), 0.4, 'energy','fontsize',16);
energyFullLength = 2.6;
line(0+[0 energyFullLength],[0.4 0.4], 'lineWidth', 5, 'color', [0.6 0.8 1]);
henergy = line(0+[0 energyFullLength],[0.4 0.4], 'lineWidth', 5);

fullHeart = 10;
numHeart = fullHeart;
blackHeartString = sprintf(strrep('\u2665  ', '\u', '\x'));
whiteHeartString = sprintf(strrep('\u2661  ', '\u', '\x'));

text(0+(-0.5), 0.7, 'health','fontsize',16);
hHeartText = text(0+0, 0.7, repmat(blackHeartString,1,numHeart),'color','r',...
    'fontsize',22);
...
hgndtiles = patch('Vertices', tilevm, 'Faces', tilefm, ...
    'EdgeColor',[.75 .7 .65], 'FaceColor',[.75 .7 .65]);
...
if(drawFromData && numsteps>drawFromData)
    disp('did not provide enough data, so drawing less steps.');
    numsteps = drawFromData;
end

if(drawFromData) % assuming that data are sampled at a fixed speed
    framecounts = cellfun('length',drawData);    
    if(treadmillVel==0)
        treadmillVel = L*(sin(drawData{1}(1,1))-sin(drawData{1}(end,1)))...
            /framecounts(1);
    end
else
    treadmillVel = sl / framecounts(1);
end
if(~treadmill), treadmillVel = 0; end;
...
energyLevel = energyFullLength;
for j = 1:numsteps % Loop through each step
  % finishLikeStart adds +1 step, to end on frame similar to beginning
  set(henergy, 'XData', 0+[0 energyLevel]);
  if(~isempty(falls)&&j>1&&falls(j-1))
     if(treadmill)
        Walk_distance = 0;     
     else
        for h_itr=11:14
        set(h(h_itr), 'Color', [get(h(h_itr),'Color') 0.5]) 
        end 
        set(h(15), 'markerFaceColor', [.4 .4 .4])
        pause(framedelay*10)
        
        h = drawmodel(w, xs(1,:), [modelorigin;0], [], drawmodeloptions{:});        
        drawnow;        
     end
     numHeart = numHeart -1;
     set(hHeartText, 'String', [repmat(blackHeartString,1,numHeart), ...
         repmat(whiteHeartString,1,fullHeart-numHeart)]);
  elseif(j>1)
      plot(Walk_distance-R*sin(xs(end,2)), 0, '.', 'markerSize', 20, ...
          'color', [.6 .55 .4]);
  end
  
  modelorigin = Walk_distance;
  modelorigin_h = Walk_distance_h;
  ...
  % update distance before (possibly) cutting out first frame
  if(~isempty(falls) && falls(j))
      Walk_distance = Walk_distance + sl; %if fell, re-start from sl away
  else
      Walk_distance = Walk_distance + R*(xs(1,1)-xs(end,1))+...
          (L-R)*(sin(xs(end,2))-sin(xs(end,1)));
  end
  
  % if previous step was not fall, cut out one frame
  if(~isempty(falls) && j>1 && ~falls(j-1))
      initFrame = 2;
  else
      initFrame = 1;
  ...
  for i = initFrame:framesthisstep % each step has this many frames
    drawmodel(w, xs(i,1:4), [modelorigin; 0], h, drawmodeloptions{:});
    set(hgndtiles,'Vertices', tilevm); % update treadmill vertices
        
    modelorigin = modelorigin - dx; % move model to left
    modelorigin_h = modelorigin_h - dx; % move model to left
    ...
    if(mod(j,2)==0) % 15:com
        set(h(12), 'Color', [84, 12, 127]/256); 
        set(h(11), 'Color', [84, 12, 127]/256);
        set(h(13), 'Color', 'r'); set(h(14), 'Color', 'r');
    else
        set(h(12), 'Color', 'r'); set(h(11), 'Color', 'r');
        set(h(13), 'Color', [84, 12, 127]/256); 
        set(h(14), 'Color', [84, 12, 127]/256);
    end
    drawnow; 
    pause(framedelay)
...
%       (L-R)*sin(xs(end,2))+R*xs(end,2) + ...
%       -(L-R)*sin(xs(end,1))-R*xs(end,1) - treadmillVel*i;  
  energyLevel=energyLevel-stepEnergy(j)*1.8;
  
%   if(internal)
%       Walk_distance_h = Walk_distance_h + (L-R)*sin(xs(end,6))+R*xs(end,6) + ...
%           -(L-R)*sin(xs(end,5))-R*xs(end,5) - treadmillVel*i;
%       height_h = height_h + (L-R)*(cos(xs(end,5))-cos(xs(end,6)));
%   else
%       Walk_distance_h = Walk_distance;
%   end
end % numsteps loop
