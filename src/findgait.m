function [wstar,cnvrg,errmsg,wsuccess] = findgait(w1, w2, varargin)
% FINDGAIT Finds a fixed point for a walking model by doing
% a creeping gradient search, starting from a known gait.
% [wstar, cnvrg, errmsg, wsuccess] = findgait(w1, w2, x0) takes the model w2, and returns the
...
% 'minparmdiff'
% Other options may be included for gradsearch, which is called by findgait.
% 
...
% errors from gradsearch. It would help to return more informative
% information
...
while length(opt_argin) >= 2,
 ...
 switch opt
   ...
   case {'AbsTol','RelTol','tf','parmsearch'}  
     % options for gradsearch, do nothing
   otherwise
     error('Findgait options: extrapolate, parmvary, delta, info, criterion, stepsize')
 end
end
...
try
 [wstar, cnvrg] = gradsearch(w2, x0, options{:});
catch
 errmsg = lasterr;
 if info > 0
   fprintf(lasterr);
   fprintf(1,'gradsearch 1 failed\n')
 end
...
if cnvrg, % if you're already done, return successfully
 if info > 0
   fprintf('gradsearch 1 succeed, findgait return\n');
 end
 %  wsuccess = wstar;
 drawnow;
 return
...
[wintermediate, cnvrg, errmsg, wsuccess] = findgait(w1, interpolategait(w1,w2),[],options{:});
...
%           this time starting from the intermediate solution
  ...
  [wstar, cnvrg, errmsg, wothers] = findgait(wintermediate, w2, xnew, options{:});
...
if info > 0, fprintf('exit findgait\n'); end