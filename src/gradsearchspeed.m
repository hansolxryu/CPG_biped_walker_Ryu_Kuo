function [wstar,cnvrg] = gradsearchspeed(w, x0, speed0, varargin)
% GRADSEARCHSPEED Finds a fixed point for walking model by doing
% a gradient search to match a desired speed.
% [wstar, cnvrg] = gradsearch(w, x0, speed0) takes the model w, and returns the
...
delta = 1e-7; info = 1; criterion = 1e-8; stepsize = 1; parmvary = 'gamma'; tryharder = 1;
method = 'normal';
...
parms = get(w, 'parms'); 
theparameter = getfield(parms, parmvary);
%gamma = parms.gamma; 
...
while max(abs(e)) > criterion
  ...
  if calc
    y = zeros(N+1,N+1);
    y1 = onestepspeed(w, xs);
    for i=1:N
      x = xs; x(i) = x(i) + delta;
      y(:,i) = onestepspeed(w, x)'-y1';
    end
    %xc2 = onestepspeed(set(w, 'gamma', delta),xs);
    xc2 = onestepspeed(set(w, parmvary, theparameter+delta),xs);
    y(:,N+1) = xc2'  - y1'; 
    J = y / delta;
    dx = J \(-y1');
  end
  dparm = dx(N+1)*stepsize;
  xsn(1:N) = xs(1:N) + dx(1:N)'*stepsize; 
  %wtry = set(w,'gamma', gamma+dgamma);
  wtry = set(w,parmvary, theparameter + dparm);
  enew = onestepspeed(wtry, xsn);
  if max(abs(enew)) > max(abs(e)) || any(isnan(enew)) % if error not reducing, try a smaller step size
    ...
    xs = xsn;
    w = wtry;
    theparameter = theparameter + dparm; %gamma = gamma+dgamma;
    calc = 1;
    stepsize = ostepsize;
    delta = abs(delta);
  end
  ...
  wstar = set(wstar, 'xstar', xs, parmvary, theparameter); 
  scount = scount + 1;
end
...
% speed as an output
function xout = onestepspeed(wnew, xsnew)
